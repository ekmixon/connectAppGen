        ...
        with open (path) as json_file:
            ...
            try:
                for ag in data['action_groups']:
                    if not os.path.exists(
                        f"{imagesPath}np_ng/action_groups/" + ag['name'] + "/"
                    ):
                        os.makedirs(f"{imagesPath}np_ng/action_groups/" + ag['name'] + "/")
                    filename = (
                        f"{imagesPath}np_ng/action_groups/"
                        + ag['name']
                        + "/"
                        + ag['name']
                        + ".png"
            ...
            try:
                for a in data['actions']:
                    if not os.path.exists(
                        f"{imagesPath}np_ng/actions/" + a['name'] + "/"
                    ):
                        os.makedirs(f"{imagesPath}np_ng/actions/" + a['name'] + "/")

                    filename1 = (
                        f"{imagesPath}np_ng/actions/"
                        + a['name']
                        + "/"
                        + a['name']
                        + ".png"
            ...
            try:
                for g in data['groups']:
                    if not os.path.exists(
                        f"{imagesPath}np_ng/field_groups/" + g['name'] + "/"
                    ):
                        os.makedirs(f"{imagesPath}np_ng/field_groups/" + g['name'] + "/")
                    filename = (
                        f"{imagesPath}np_ng/field_groups/"
                        + g['name']
                        + "/"
                        + g['name']
                        + ".png"
            ...
            try:
                for policy in data['policy_template']['policies']:
                    if not os.path.exists(
                        f"{imagesPath}np_ng/templatedirs/"
                        + data['policy_template']['policy_template_group'][
                            'name'
                        ]
                        + "/"
                    ):
